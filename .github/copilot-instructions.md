...
- `WINNERS_CHANNEL_ID` (default `1511663782705762416`) — channel where `/event award` posts event-winner announcements (the "hall of fame").
...
- `/event` — admin command **group** (Manage Server perm, set once on the group) bundling the event/achievement actions:
    - `/event award winner:<member> event_name:<str> [event_channel:<channel>] [title:<str>] [reason:<str>]` — announce an event winner in `WINNERS_CHANNEL_ID` (gold V2 container: winner mention + event name + optional `title` — `reason` citation, plus a "View event" Link button to `event_channel` when given) and, when `title` is set, grant that **cosmetic profile title** (persisted via `analytics.award_title`). No pings (`allowed_mentions parse:[]`). Ephemeral confirm.
    - `/event revoke-title member:<member> title:<str>` — remove a cosmetic title from a member (case-insensitive match via `analytics.revoke_title`). Ephemeral confirm.
- Sync happens in `on_ready` via `tree.sync()` — currently 6 top-level commands (one, `/event`, is an `app_commands.Group` with `award` + `revoke-title` subcommands).
