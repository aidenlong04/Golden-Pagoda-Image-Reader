...
- `/profile [user] [ephemeral]` — render a member's **user profile** card PNG: the same role-derived reference grid as `/progress` (Clan | Platform over Mastery Rank | Syndicate) but **without the progress bar**, under a refined gold "USER PROFILE" eyebrow + name header. Defaults to the caller; `ephemeral` hides the reply. Any member can run it.
...
- **Profile card** (`_render_profile_card_png` in `bot.py`): a sibling of the progress card with the bar removed — same supersampled rounded slate panel, circular avatar and shared `_draw_info_grid` two-column reference grid, but a refined header of a gold "USER PROFILE" eyebrow above the member name (no bar/percent/count to anchor it). Its grid is purely role-derived (no OCR), laid out row-major as `Clan | Platform` over `Mastery Rank | Syndicate`; categories the member hasn't earned render an em-dash "—" so the 2×2 stays balanced. Rows come from the async `_member_profile_info_lines(member)` gatherer (Clan slot name + emoji, platform + `PLATFORM_EMOJI_*`, mastery role name + `MASTERY_RANK_EMOJI`, syndicate role names joined + `SYNDICATE_EMOJI`). `/profile` defers then sends just the `profile.png` image (no V2 container).
