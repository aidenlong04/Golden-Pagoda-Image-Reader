...
- **Progress card**: `_render_progress_card_png` is laid out in logical units and rendered at `_PROGRESS_SS`× (supersampled, crisp on HiDPI). Composites a circular avatar + a numpy-shaded **segmented** bar (`_segmented_bar` — one rounded segment per category; filled segments share a continuous glassy gradient with gloss/traced outline/leading-edge glow, empty segments are amber-tinted "pending" track; gold when complete) + `(label, value, emoji_bytes)` info rows. Row icons are aspect-preserved (`ImageOps.contain`, centered, via shared `_paste_emoji_icon`) and use the configured custom emoji (via `_fetch_emoji_bytes` at 128px, cached per emoji ID) instead of a bullet — fed by `CLAN_ROLE_*_EMOJI` / `PLATFORM_EMOJI_*` / `OPERATOR_EMOJI` / `MASTERY_RANK_EMOJI` / `WARNING_EMOJI`. A status line under the bar shows the gold complete note or an amber "Missing: …" pill; reference rows lay out in a two-column grid (shared `_draw_cell` helper, row-major: Profile | Mastery over Clan | Platform) below the divider. On a pass, the card image sits on top as a top-level media gallery (type 12) and `_pass_components` builds ONE gold V2 container (type 17) holding every button in a single row — the call-sign `nick:` choices (when a name is worth suggesting) plus the "Pick Roles" Link button — so the reply is one image + one container (the nick prompt is folded in via the shared `_callsign_buttons` helper — caption `_CALLSIGN_CAPTION` + `_nick_button` choices, reused by the card path, the no-card fallback, and `_nickname_prompt_components`; `_strip_nick_prompt` later drops any standalone `_NICK_PROMPT_ACCENT` container and removes just the caption + nick buttons from the pass container, keeping Pick Roles). The incomplete reply's `_nickname_prompt_components` is its own self-contained gold container. All Link buttons go through the shared `_link_button` / `_link_button_row` helpers.
