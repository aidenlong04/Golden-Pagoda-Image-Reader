...
- **Progress card**: `_render_progress_card_png` is laid out in logical units and rendered at `_PROGRESS_SS`× (supersampled, crisp on HiDPI). Composites a circular avatar + a numpy-shaded capsule bar (shading blend, gloss, traced outline, leading-edge glow; gold when complete) + `(label, value, emoji_bytes)` info rows. Row icons are aspect-preserved (`ImageOps.contain`, centered) and use the configured custom emoji (via `_fetch_emoji_bytes` at 128px, cached per emoji ID) instead of a bullet — fed by `CLAN_ROLE_*_EMOJI` / `PLATFORM_EMOJI_*` / `OPERATOR_EMOJI` / `MASTERY_RANK_EMOJI` / `WARNING_EMOJI`. Regular rows lay out in a two-column grid (shared `_draw_cell` helper) to fill the width; "Missing Data" spans full width last and larger. On a pass, the card image sits on top as a top-level media gallery (type 12) and the "Pick Roles" Link button renders below it inside one gold V2 container (type 17) so the button always sends. All Link buttons go through the shared `_link_button` / `_link_button_row` helpers.
