...
- **Health signal**: bot writes `/tmp/gp_health` every `HEALTH_INTERVAL` seconds (default 20s). Dockerfile `HEALTHCHECK` marks unhealthy when file is stale (>90s). Watchdog timer `golden-pagoda-watchdog` polls `docker inspect ... .State.Health.Status` every 60s and restarts on `unhealthy`/`missing`.
