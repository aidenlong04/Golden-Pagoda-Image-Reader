...
- **Progress card**: `_render_progress_card_png` is laid out in logical units and rendered at `_PROGRESS_SS`× (supersampled, crisp on HiDPI). Composites a circular avatar + a numpy-shaded **segmented** bar (`_segmented_bar` — one rounded segment per category; filled segments share a continuous glassy gradient with gloss/traced outline/leading-edge glow, empty segments are amber-tinted "pending" track; gold when complete) + `(label, value, emoji_bytes)` info rows. Row icons are aspect-preserved (`ImageOps.contain`, centered, via shared `_paste_emoji_icon`) and use the configured custom emoji (via `_fetch_emoji_bytes` at 128px, cached per emoji ID) instead of a bullet — fed by `CLAN_ROLE_*_EMOJI` / `PLATFORM_EMOJI_*` / `OPERATOR_EMOJI` / `MASTERY_RANK_EMOJI` / `WARNING_EMOJI`. A status line under the bar shows the gold complete note or an amber "Missing: …" pill; reference rows lay out in a two-column grid (shared `_draw_cell` helper) below the divider. On a pass, the card image sits on top as a top-level media gallery (type 12) and the "Pick Roles" Link button renders below it inside one gold V2 container (type 17) so the button always sends. All Link buttons go through the shared `_link_button` / `_link_button_row` helpers.
